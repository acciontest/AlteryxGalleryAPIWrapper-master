...
namespace AlteryxGalleryAPIWrapper
{
    public class Client
    {
        ...
        public string UnRestrictApp(string appinterface, string appid)
        {
            HttpWebRequest req = (HttpWebRequest)WebRequest.Create(this.url + "/curator/apps/"+ appid +"/");
            req.ContentType = "text/json";
            req.Method = "PUT";
            req.Headers.Add("Authorization", string.Format("SPECIAL {0}", this.sessionId));
            var response = HTTPClient.DispatchRequest(req, appinterface, "PUT");
            return response.stream;
        }
